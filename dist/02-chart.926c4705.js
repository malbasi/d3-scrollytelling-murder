...
},{}],"data/phillymurderrate.csv":[function(require,module,exports) {
module.exports = "/phillymurderrate.e0facd26.csv";
...
var xPositionScale = d3.scaleBand().range([100, width]);

var forceXCombine = d3.forceX(function (d) {
  return width / 2;
}).strength(0.05);

var forceXRace = d3.forceX(function (d) {
  return xPositionScale(d.race);
}).strength(0.05);

var forceYClosed = d3.forceY(function (d) {
  if (d.closed === 'yes') {
    return height / 1.1;
  } else {
    return height / 2;
...
var simulation = d3.forceSimulation().force('x', forceXCombine).force('y', forceYCombine).force('collide', d3.forceCollide(6)).force('charge', d3.forceManyBody().strength(-2));

var colorScale = d3.scaleOrdinal().range(['#ef5675', '#003f5c', '#58508d']);

d3.csv(require('./data/phillymurderrate.csv')).then(ready).catch(function (err) {
  return console.log('Failed on', err);
});

function ready(datapoints) {
  var race = datapoints.map(function (d) {
    return d.race;
  });
  colorScale.domain(race);
  xPositionScale.domain(race);

  var circles = svg.selectAll('.populations').data(datapoints).enter().append('circle').attr('class', 'populations').attr('r', 5).attr('fill', '#808080');

  d3.select('#all-pop').on('stepin', function () {
    svg.selectAll('.populations').transition().duration(1000).attr('fill', function (d) {
  ...
  d3.select('#pop-race').on('stepin', function () {
    console.log('pop-race stepin');
    simulation.force('x', forceXRace).force('y', forceYCombine).alphaTarget(0.5).restart();
  });

  d3.select('#murder-rate').on('stepin', function () {
    console.log('murder rate step in');
    simulation.force('y', forceYClosed).alphaTarget(0.2).restart();
  });
...
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","debounce":"../node_modules/debounce/index.js","./data/phillymurderrate.csv":"data/phillymurderrate.csv"}],"../../../../../../usr/local/lib/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../../../../../../usr/local/lib/node_modules/parcel/src/builtins/hmr-runtime.js","02-chart.js"], null)
//# sourceMappingURL=/02-chart.926c4705.map